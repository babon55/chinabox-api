...
chinaexpress — это REST API, построенный на **Fastify**, **Prisma** и **PostgreSQL**. Это бэкенд для электронной коммерции туркменского рынка с поддержкой двух языков (туркменский/русский). API включает аутентификацию, управление товарами, заказами, клиентами и загрузку файлов (Cloudinary).
