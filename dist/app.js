...
export async function buildApp() {
    ...
    // ── Rate Limiting ─────────────────────────────────────────────────────────────
    // Global default limit from config
    // Construct options; cast to any to bypass incomplete plugin types
    const rlOptions = {
        ...
        skip: (req) => {
            const path = req.url ?? '';
            return path === '/health' || path.startsWith('/docs') || path.startsWith('/api/v1/docs');
        },
    };
    await app.register(rateLimit, rlOptions);
