...
export default async function authRoutes(app) {
    ...
    app.post('/refresh', {
        ...
        if (!parsed.success)
            return badRequest(reply, 'refreshToken required');
    ...
    app.patch('/me', {
        ...
        const user = await app.prisma.user.update({ where: { id: req.user.sub }, data });
        return reply.code(200).send(user);
    });
    ...
    app.post('/change-password', {
        ...
        await app.prisma.refreshToken.deleteMany({ where: { userId: req.user.sub } });
