...
export default async function customerAuthRoutes(app) {
    ...
    app.post('/login', {
        ...
        if (customer.status === 'BLOCKED')
            return unauthorized(reply, 'Account is blocked');
        const payload = { sub: customer.id, email: customer.email, role: 'CUSTOMER' };
        const accessToken = app.jwt.sign({ ...payload, type: 'access' }, { expiresIn: config.jwt.accessExpiresIn });
        const refreshToken = app.jwt.sign({ ...payload, type: 'refresh' }, { expiresIn: config.jwt.refreshExpiresIn });
    ...
    app.post('/refresh', {
        ...
        if (!refreshToken)
            return badRequest(reply, 'Refresh token required');
        let payload;
        try {
        ...
        if (!customer)
            return unauthorized(reply, 'Customer not found');
        if (customer.status === 'BLOCKED')
            return unauthorized(reply, 'Account is blocked');
...
//# sourceMappingURL=custumer-auth.routes.js.map