...
export default async function ordersRoutes(app) {
    ...
    app.get('/', {
        ...
        if (!q.success)
            return badRequest(reply, q.error.message);
    ...
    app.post('/', {
        ...
        for (const line of lines) {
            ...
            if (!product)
                return badRequest(reply, `Invalid product ID: ${line.productId}`);
            ...
            const optionsArray = Array.isArray(product.options)
                ...
                : typeof product.options === 'string'
                    ...
                    : [];
            const requiredOptions = optionsArray.filter((o) => o.required !== false);
